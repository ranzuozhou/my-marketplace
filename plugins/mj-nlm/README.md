...
| `/mj-nlm:studio` | Studio 制品生成（9 种类型 × **4 view 视角** + **默认幻觉防护**） |
| `/mj-nlm:learn` | **学习闭环编排器**：10 Phase + 5 Gate 一键走完「来源准备 → 领域定向 → mind_map → video → slide → audio → quiz → 错题 root cause → 来源核查 → 理解度仪表盘」 |
